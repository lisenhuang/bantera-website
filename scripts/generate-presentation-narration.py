...
Usage: python3 scripts/generate-presentation-narration.py [segment-id ...] [--manifest-only]
...
for seg in segments:
    ...
    if seg.get('kind')!='video':
        ...
        item.update(kind='audio',src='/presentation/narration/'+dest.name+'?v='+fingerprint[:12],duration=round(duration,3))
    manifest.append(item)
(ROOT/'src/app/presentation/narration.json').write_text(json.dumps(manifest,ensure_ascii=False,indent=2)+'\n')
